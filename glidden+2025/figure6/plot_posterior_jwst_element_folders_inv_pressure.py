...
# Get JWST outp
element = 'CH4'
folder = 'JWST-GHOSTMU-R10000'

if 'GHOST' in folder:

    results_JWST = pickle.load(open('../../espinoza+2025/figure3/retrieval-results/'+folder+'/data_multi_visit_espinoza_gp_ghost-ghostatmosphere_george_results.pkl','rb'))

    T,  CO2, CH4, H2O, N2, O2, O3, N2O, CO, jwst_H2, jwst_Pcloud, offset1, rho1, gp_sigma1, sigma_w1, \
        offset2, rho2, gp_sigma2, sigma_w2,\
        offset3, rho3, gp_sigma3, sigma_w3,\
        offset4, rho4, gp_sigma4, sigma_w4, P_ref, mu =  results_JWST['posterior_samples'].T

else:

    results_JWST = pickle.load(open('../../espinoza+2025/figure3/retrieval-results/'+folder+'/data_multi_visit_espinoza_gp_H2-H2atmosphere_george_results.pkl', 'rb'))
...
print('saving to','posteriors_jwst_log_'+element+'_'+folder+'.pdf')
plt.savefig('posteriors_jwst_log_'+element+'_'+folder+'-ip.pdf',
