...
for name, index in zip(['visit1', 'visit2', 'visit3', 'visit4'], [0, 1, 2, 3]):
...
# Set x-labels:
for name in ['visit1', 'visit2', 'visit3', 'visit4']:
...
for name in ['visit1', 'visit2', 'visit3', 'visit4']:
    ...
    ax[name].set_xlim(0.75, 5.0)
    ax[name].set_ylim(4300,5500)

folder = 'retrieval-results/JWST-CLR-R10000'
...
atmosphere_color = 'cornflowerblue'
flat_line_color = 'darkorange'
ms = 8
ew = 2
alpha = 1.0

nmodels = np.load(folder+'/data_multi_visit_espinoza_gp_H2-H2atmosphere_george_binned_posterior_contamination1.npy').shape[0]
# data_multi_visit_espinoza_gp_FLATLINE-FLATLINEatmosphere_george_binned_posterior_contamination1
nmodels_fl = np.load(folder+'/data_multi_visit_espinoza_gp_FLATLINE-FLATLINEatmosphere_george_binned_posterior_contamination1.npy').shape[0]
for name, date in zip(['1', '2', '3', '4'], ['June 22', 'June 28', 'July 23', 'October 28']):

    # Print date:
    ax['visit'+name].text(3.5,4400,date+', 2023', fontsize = 30)

    # Read data:
    w_ne, d_ne, derr_ne = np.loadtxt('data/jwst/visit'+name+'.txt', unpack = True, usecols = (0, 3, 4)) 

    # Extract posterior model samples from both the spectrum and the contamination component:
    contamination = np.load(folder+'/data_multi_visit_espinoza_gp_H2-H2atmosphere_george_binned_posterior_contamination'+name+'.npy')
    model = np.load(folder+'/data_multi_visit_espinoza_gp_H2-H2atmosphere_george_binned_posterior_spectrum'+name+'.npy')
    full_model = contamination * model

    contamination_fl = np.load(folder+'/data_multi_visit_espinoza_gp_FLATLINE-FLATLINEatmosphere_george_binned_posterior_contamination'+name+'.npy')
    model_fl = np.load(folder+'/data_multi_visit_espinoza_gp_FLATLINE-FLATLINEatmosphere_george_binned_posterior_spectrum'+name+'.npy')
    full_model_fl = contamination_fl * model_fl
    ...
    full_model_fl_median = np.zeros(full_model.shape[1])
    full_model_fl_1sigma_up = np.zeros(full_model.shape[1])
    full_model_fl_1sigma_down = np.zeros(full_model.shape[1])
    full_model_fl_3sigma_up = np.zeros(full_model.shape[1])
    full_model_fl_3sigma_down = np.zeros(full_model.shape[1])
    ...
    for i in range(full_model.shape[1]):
        ...
        full_model_fl_median[i], full_model_fl_1sigma_up[i], full_model_fl_1sigma_down[i] = juliet.utils.get_quantiles(full_model_fl[:, i])

        _, full_model_3sigma_up[i], full_model_3sigma_down[i] = juliet.utils.get_quantiles(full_model[:, i], alpha = 0.99)
        _, full_model_fl_3sigma_up[i], full_model_fl_3sigma_down[i] = juliet.utils.get_quantiles(full_model_fl[:, i], alpha = 0.99)
    ...
    ax['visit'+name].plot(w_ne, full_model_fl_median, lw = 6, color = flat_line_color, zorder = 3)
    ax['visit'+name].plot(w_ne, median_contamination_fl - 250, '--', lw = 3, color = flat_line_color, zorder = 3)
    ...
    ax['visit'+name].fill_between(w_ne, full_model_fl_1sigma_down, full_model_fl_1sigma_up, color = flat_line_color, alpha = 0.3, zorder = 1)
    ax['visit'+name].fill_between(w_ne, full_model_fl_3sigma_down, full_model_fl_3sigma_up, color = flat_line_color, alpha = 0.2, zorder = 1)
