...
atmosphere_color = '#1E90FF'
band_atmosphere_line_color = '#87CEFA'
flat_line_color = '#FF8500'
band_flat_line_color = '#FFB84D'
bf_model = 'flat'


folder = 'retrieval-results/JWST-CLR-R10000'
plot_earth_like = True

if plot_earth_like:

    earth_like_color = 'forestgreen'
    w_el, d_el = np.loadtxt('TRAPPIST1e_noerror_earthlike.csv', unpack = True, usecols = (0,1), delimiter = ',')
...
if 'ghostmu' in folder:
    ...
    prefix = 'data_multi_visit_espinoza_gp_H2-H2atmosphere'
...
for name, date in zip(['1', '2', '3', '4'], ['June 22', 'June 28', 'July 23', 'October 28']):
    ...
    # Median removal:
    median_value = np.nanmedian( model_fl )#corrected_spectra )

    if counter == 0:

        combined_models = (full_model / median_contamination ) - median_value
        combined_models_fl = ( full_model_fl / median_contamination ) - median_value
        ...
        combined_models_fl = np.vstack(( combined_models_fl, ( full_model_fl / median_contamination ) - median_value ))
...
m_fl = np.zeros(len(w_ne))
m_fl_1sigma_up = np.zeros(len(w_ne))
m_fl_3sigma_up = np.zeros(len(w_ne))
m_fl_1sigma_down = np.zeros(len(w_ne))
m_fl_3sigma_down = np.zeros(len(w_ne))
for i in range(len(w_ne)):
    ...
    m_fl[i], m_fl_1sigma_up[i], m_fl_1sigma_down[i] = juliet.utils.get_quantiles(combined_models_fl[:, i]) 
    _, m_fl_3sigma_up[i], m_fl_3sigma_down[i] = juliet.utils.get_quantiles(combined_models_fl[:, i], alpha = 0.99) 


# Now plot median models for both:
plt.plot(w_ne, m, lw = 7, color = atmosphere_color, zorder = 6)

plt.plot(w_ne, m_fl*0, lw = 7, color = flat_line_color, zorder = 7)
...
plt.fill_between(w_ne, m_fl_1sigma_down, m_fl_1sigma_up, color = band_flat_line_color, alpha = alphas, zorder = 3)
plt.fill_between(w_ne, m_fl_3sigma_down, m_fl_3sigma_up, color = band_flat_line_color, alpha = alphas, zorder = 3)
# Plot data:
